...
def callScoreingphpscript(inputtable,outputtable,pathwayfile,targetid):
    # print(pathwayfile+"           start")
    result = subprocess.run(
        ['C:/wamp64/bin/php/php5.6.40/php.exe', 'C:/Users/whl19/Documents/Code/GenebetweenPathways/pathwayscoreUtilies/score_pathways_cliforSpecificialP2.php',inputtable,outputtable,pathwayfile,targetid],    # program and arguments
    ...
    while True:       
        ...
        for j in range(numthreads):
            if i+j<len(pathwaypathlist): 
                ...
                tfid = pathwaynamelist[i+j].split("_")[2]
                ...
                p = Process(target = callScoreingphpscript,args=(inputtable,outputtable,pathwaypathlist[i+j],tfid))
                p.start()
                l.append(p) 
        for p in l :
            p.join() 
...
if __name__=="__main__":
    ...
    numthreads = 1
    outputdir=allresultdir+"revisedScoring_test/"
    if not os.path.exists(outputdir):
        os.makedirs(outputdir)
    multipleCallscoringscript(revisedpathwaydir,numthreads,inputtable,outputdir)
